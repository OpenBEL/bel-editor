...
public enum ReturnType {
    ...
    public static boolean assignableFrom(final ReturnType supertype,
        ...
        switch (supertype) {
        case ABUNDANCE:
            if (subtype == GENE_ABUNDANCE)
                return true;
            if (subtype == MICRORNA_ABUNDANCE)
                return true;
            if (subtype == PROTEIN_ABUNDANCE)
                return true;
            if (subtype == COMPLEX_ABUNDANCE)
                return true;
            if (subtype == RNA_ABUNDANCE)
                return true;
            if (subtype == ABUNDANCE)
                return true;
            break;
        case BIOLOGICAL_PROCESS:
            if (subtype == PATHOLOGY)
                return true;
            if (subtype == BIOLOGICAL_PROCESS)
                return true;
            break;
        case GENE_ABUNDANCE:
            if (subtype == GENE_ABUNDANCE)
        ...
        case PROTEIN_MODIFICATION:
            if (subtype == PROTEIN_MODIFICATION)
                return true;
            break;
        case REACTANTS:
            if (subtype == REACTANTS)
                return true;
            break;
        case SUBSTITUTION:
